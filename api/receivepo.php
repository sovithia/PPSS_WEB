...
function createPO($items,$author,$fromPO = null,$notes = null,$vendid = null)
{
	...
	$PURCHASE_AMT = round($PURCHASE_AMT,4);
	$CURRENCY_VATAMOUNT = $VAT_AMT;
	$CURRENCY_VATAMOUNT = round($CURRENCY_VATAMOUNT,4);
	$CURRENCY_AMOUNT = round($PURCHASE_AMT, 4); //+ $VAT_AMT;
...
{	
	...
	foreach($items as $item){
		...
		if ($item["PPSS_DELIVERED_EXPIRE"] == "NO EXPIRE")
...
{			
	...
	$INV_AMT =  round($PORef["CURRENCY_AMOUNT"], 4); //+ $PORef["VAT_AMT"];
	$PAID_AMT =  "0";
	$BALANCE =  round($PORef["CURRENCY_AMOUNT"], 4); //+ $PORef["VAT_AMT"];
	$PCNAME =  "Application";
	$CURR_RATE = "1";	
	$REMARK =  $notes;
	$TERMID =  "";
	...
	$CURR_ID =   "USD";
	$BASECURR_ID =  "USD";
	$CURRENCY_AMOUNT = round($PORef["CURRENCY_AMOUNT"], 4); //+ $PORef["VAT_AMT"]; 
	$CURRENCY_VATAMOUNT =  round($PORef["CURRENCY_VATAMOUNT"], 4); 
	$SHIP_REFERENCE = "";
	$DATEADD =  $today;
	$USERADD = blueUser($author);
    ...
    {		
		...
		$OPERATIONBASE =   "";
		$CURRID_EXCHRATE = "1";
		$CURRENCY_AMOUNT = round($item["CURRENCY_AMOUNT"], 4);

		$COST_ADD =  "0";
		$CURR_ID =   "USD";
		$BASECURR_ID = "USD";
		$CURRENCY_COST = round($item["CURRENCY_COST"], 4);
		$CURRENCY_COST_ADD =  "0";
		$ORIGINAL_QTY =  "0";
		$QTY_PACK =     "0";
		$PACK_WEIGHT =  "0";
	...
	$USER_DOCNO = "";
	$SHIP_REFERENCE = "";
	$CURRENCY_AMOUNT_SUM = round($CURRENCY_AMOUNT_SUM,4);
	...
	$GRANDTOTAL = round(($CURRENCY_AMOUNT_SUM + $VAT_SUM), 4);	
	...
	$ARACC =    '';
	$BASECURR_ID =   "USD";
	$CURRENCY_AMOUNT = round($GRANDTOTAL, 4);
	$PURPOSE_ISSUE =  '';
	$JOB_ID =  '';
	$USERADD =    blueUser($author);
	$DATEADD =   $today;
	...
	{		
		...
		$TRANEXTCOST_NEW =  round($item["ORDER_QTY"] * $item["TRANCOST"], 4);
		$LINE_DISCAMT = "0";
		$COST_CENTER = "";
		$LINE_NOTE = "";
		$CASE_PRODUCTID = "";
	...
	$INV_AMT =      round($theTOTALAMOUNT, 4);// + $theVATAMOUNT;
	$PAID_AMT =      "0";
	$BALANCE =      round($theTOTALAMOUNT, 4);// + $theVATAMOUNT;
	$PCNAME =      "APPLICATION";
	$CURR_RATE =    "1";   
	$REMARK =      $notes;
	$TERMID =      '';
	...
	$CURR_ID =      "USD";
	$BASECURR_ID =   "USD";
	$CURRENCY_AMOUNT =   round($theTOTALAMOUNT,4);// + $theVATAMOUNT;;
	$CURRENCY_VATAMOUNT =  $theVATAMOUNT;
	$CURRENCY_VATAMOUNT = round($CURRENCY_VATAMOUNT,4);
	$CURRENCY_BALANCE =  $theTOTALAMOUNT;// + $theVATAMOUNT;
	$CURRENCY_BALANCE = round($CURRENCY_BALANCE,4);
	$CURRENCY_PAIDAMT =   "0";
	$DATEADD =   $today;
	$USERADD = 	blueUser($author);
	...
	foreach($items as $item)
	{
		...
		$CURRENCY_AMOUNT = round($item["CURRENCY_AMOUNT"], 4);
		$CURRENCY_COST = $item["CURRENCY_COST"];
		$AMOUNT =  round($item["CURRENCY_AMOUNT"], 4);
		$CURR_RATE =  "1";
		$DATEADD =   $today;
		$USERADD =   blueUser($author);
	...
	$GLMONTH =    date("m"); 
	$ACCNO =    "20000";
	$GLAMT =    round($theTOTALAMOUNT, 4);//+ $theVATAMOUNT; 
	$DEBIT =     "0";
	$CREDIT =    round($theTOTALAMOUNT, 4);// + $theVATAMOUNT;
	...
	if ($HAVEVAT)
	{
		...
		$GLMONTH =  date('m');
		$ACCNO =     "16100";
		$GLAMT =    round($theVATAMOUNT, 4);
		$DEBIT =     round($theVATAMOUNT, 4);
		$CREDIT =    0;
		$DOCNO =    sprintf("VO%013d",$APNUM);
		$USERADD =    blueUser($author);
		$DATEADD =    $today;
	...
	//if ($HAVEVAT)		
		//$GLAMT = $theVATAMOUNT;
	//else 
		$GLAMT = round(($theTOTALAMOUNT - $theVATAMOUNT), 4);
	
	//if ($HAVEVAT)
	//	$DEBIT = $theVATAMOUNT;
	//else 
		$DEBIT = round(($theTOTALAMOUNT - $theVATAMOUNT), 4);
	...
	$LINENUM =   "1";
	$ACCNO =       "17000";
	$AMOUNT = round($theTOTALAMOUNT, 4); //+ $theVATAMOUNT;
	$PERIOD = date("n");
	$YEAR = date("Y");
	$BATCH = '';
	$REFERENCE = '';
	...
	$COMMENT_ON_LINE = '';
	$CURR_TYPE =  '';
	$CURR_AMOUNT = round($theTOTALAMOUNT, 4); //+ $theVATAMOUNT;; 
	$CURR_RATE =     "1";
	$OPERATION_BASE = '';
	$BASECURR_ID = "USD";
	$CURR_ID = "USD";
	...
	if ($HAVEVAT)
	{
		...
		$CURR_AMOUNT = round($CURR_AMOUNT,4);
		$CURR_RATE =     "1";
		$OPERATION_BASE = '';
		$BASECURR_ID = "USD";
		$CURR_ID = "USD";
